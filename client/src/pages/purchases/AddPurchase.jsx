...
// Latest rule (same as AddInvoice): (qty||1) * (boxQty||1)
...
export default function AddPurchase() {
  ...
  const [head, setHead] = useState({
    ...
    details: "",
    vatPercent: 0, // header-level %
    discountPercent: 0, // header-level %
    paidAmount: 0,
  });

  // rows
  const mkRow = () => ({
    medicineId: "",       // <-- keep and send to backend
    medicineName: "",
    batchId: "",
    expiryDate: "",
    availQty: 0, // readonly hint (current stock)
    boxPattern: "",
    unitsPerBox: 1, // for storage only
    boxQty: 0,
    quantity: 0,
    supplierPrice: 0,
    boxMRP: 0,
  ...
  // Rich medicine apply (id + fields)
  const applyMedToRow = (i, m) => {
    setRow(i, {
      medicineId: m._id,                             // <-- IMPORTANT: send id
      medicineName: m.name || rows[i].medicineName,
      availQty: pickAvail(m),                        // totalUnits
      supplierPrice: toNum(m.supplierPrice ?? m.purchasePrice ?? 0),
      // boxMRP stays user-input
    });
  };

  // custom suggester (same UX as AddInvoice)
  ...
  /* ---------- totals (using latest multiplicative rule) ---------- */
  ...
  const submit = async (e) => {
    ...
    try {
      const payload = {
        ...
        items: rows.map((r) => ({
          medicineId: r.medicineId || null, // <-- send id
        ...
        vatAmount: Number(totals.vatAmt.toFixed(2)),
        discountAmount: Number(totals.discAmt.toFixed(2)),
  ...
  }, [rows.length, leafs.length, meds.length, head.vatPercent, head.discountPercent, head.paidAmount]);
              ...
              <table className="table min-w-[1280px] w-full">
                ...
                <tbody className="text-[13px]">
                  {rows.map((r, i) => {
                    ...
                    return (
                      <tr key={i}>
                        {/* Medicine + suggester (same UX as AddInvoice) */}
