...
export default function CustomerPaid() {
  ...
  // 1) Paid / settled (balance <= 0)
  const paidOnly = useMemo(
    () => customers.filter((c) => num(c.previousBalance) <= 0),
    [customers]
  );

  // 2) Search within the paid subset
  const searched = useMemo(() => {
    if (!query) return paidOnly;
    const q = normalized(query);
    return paidOnly.filter((c) => {
      const fields = [
        c.name,
        c.mobile,
        c.email1,
  ...
  // For info: how much customers have in advance (negative balances)
  const totalAdvance = useMemo(
    () => paidOnly.reduce((sum, c) => sum + Math.min(0, num(c.previousBalance)), 0),
    [paidOnly]
  );
    ...
    <div className="min-h-screen flex bg-base-200">
      ...
      <div className="flex-1 p-4 md:p-6">
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-2xl md:text-4xl font-bold text-primary">Paid Customers</h2>
            <p className="text-sm text-gray-500">
              Showing customers with balance ≤ 0. Customers’ advance/overpayment total:{" "}
              <span className="font-semibold">{numberFmt(totalAdvance)}</span>
            </p>
          </div>
