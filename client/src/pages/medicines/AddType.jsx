...
export default function AddType() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      await axios.post(API_ADD_TYPE, { name, status: form.status });
      navigate(ROUTES.list);
    } catch (err) {
      const msg = err?.response?.data?.message || err.message || "Failed to add type";
      console.error("AddType error:", err?.response?.status, err?.response?.data, err);
      alert("❌ " + msg);
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="flex-1 p-6">
        <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-xl p-6 md:p-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl md:text-3xl font-bold text-primary">🧩 Add Type</h2>
          ...
          <form onSubmit={handleSubmit} className="max-w-3xl">
            <div className="form-control mb-6">
              <label className="label font-semibold">
                <span>Type Name <span className="text-red-500">*</span></span>
              </label>
              <input
                name="name"
