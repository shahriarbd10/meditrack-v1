...
function StatusBadge({ value }) {
  ...
  return <span className={`badge ${isActive ? "badge-success" : "badge-ghost"}`}>{isActive ? "Active" : "Inactive"}</span>;
}
...
export default function TypeList() {
  ...
  const fetchRows = async () => {
    try {
      ...
      console.error(e);
      setErr(e?.response?.data?.message || "Failed to load types");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.error(e);
      alert(e?.response?.data?.message || "Failed to delete type");
    }
  };
      ...
      <main className="flex-1 p-6">
        <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-xl p-6 md:p-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-6">
            <h2 className="text-2xl md:text-3xl font-bold text-primary">🧩 Type List</h2>
            <Link to="/dashboard/admin/medicines/type/add" className="btn btn-primary btn-sm md:btn-md">
              + Add Type
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
            ...
            <div className="join w-full">
              <button type="button" className={`btn join-item ${sortBy === "createdAt" ? "btn-active" : ""}`} onClick={() => toggleSort("createdAt")}>
                Created {sortBy === "createdAt" ? (sortDir === "asc" ? "↑" : "↓") : ""}
              </button>
              <button type="button" className={`btn join-item ${sortBy === "name" ? "btn-active" : ""}`} onClick={() => toggleSort("name")}>
            ...
            <table className="table table-zebra">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={5}><div className="py-10 text-center">No types found.</div></td></tr>
                ) : (
                  pageRows.map((r, i) => (
                    <tr key={r._id}>
